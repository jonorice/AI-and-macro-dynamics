...
\begin{abstract}
Specialised AI hardware becomes economically obsolete far faster than conventional capital, so sustaining a given stock of compute requires a high rate of replacement investment. This paper studies what that asymmetry implies for growth, adjustment dynamics, and policy in a two asset growth model in which AI capacity both raises productivity and produces low marginal cost digital services. Calibrated to advanced economies, the model delivers two distinct adjustment speeds, with AI capacity reverting quickly with a half life of about seven quarters, while conventional capital adjusts over roughly a decade. A key implication is that when hardware is short lived, modest shifts in gross spending can generate large swings in measured AI investment while leaving the underlying stock close to its long run target, which helps rationalise the volatile investment cycles seen in specialised AI hardware. Hardware durability therefore has first order welfare consequences. A two percentage point fall in quarterly depreciation raises welfare by 0.36\% in consumption equivalent terms, while an equal sized compute tax reduces the steady state AI stock by around one fifth.
\end{abstract}
...
\textbf{Keywords:} artificial intelligence, capital depreciation, two-asset growth model, hardware obsolescence, investment dynamics, welfare
...
\begin{proof}
We analyse each component of \eqref{eq:GM}.
