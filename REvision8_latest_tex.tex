...
steady state following a 1\% increase in $M_0$. Solid lines: linear GE dynamics from
\eqref{eq:GE_system}. Circle markers: partial equilibrium geometric benchmark
\eqref{eq:PE_recursions}. In the baseline calibration the PE benchmark is visually indistinguishable from the GE response at this scale and therefore lies almost entirely underneath the solid line.
\end{minipage}
\end{figure}
...
In the baseline calibration the PE benchmark is visually indistinguishable from the GE response at this scale and therefore lies almost entirely underneath the solid line.
\end{minipage}
\end{figure}
...
\begin{minipage}{0.9\textwidth}
\footnotesize
Notes: The figure plots $M_t/M^{*}$ under the GE dynamics
\eqref{eq:GE_system} (solid line) and the PE diffusion benchmark
\eqref{eq:PE_diffusion} (circle markers) for an economy starting from $M_0 \approx 0$.
Under the baseline calibration, about 90\% of the steady-state AI stock is reached within
roughly six years. The PE benchmark is visually indistinguishable from the GE response at this scale and therefore lies almost entirely underneath the solid line.
\end{minipage}
\end{figure}
