...
\begin{abstract}
Specialised AI hardware becomes economically obsolete far faster than conventional capital. This paper embeds that distinction in a tractable growth model with two capital stocks, conventional capital and AI capacity interpreted as installed compute used to deploy existing models. AI capacity both raises productivity in goods production and supplies digital services that are cheap to use once installed but constrained by installed capacity. The steady state is pinned down by user cost conditions. Local dynamics are obtained from a corrected linearisation in which investment is endogenous along transitions, yielding a second-order Euler block whose stable manifold defines a two-dimensional law of motion for the capital stocks. The resulting general-equilibrium impulse responses diverge markedly from a partial-equilibrium benchmark that holds investment at replacement levels: because replacement flows are large relative to small stock deviations for short-lived AI hardware, the planner can close stock gaps much faster than passive depreciation-driven decay. The PE benchmark nevertheless provides a transparent yardstick: calibrated to advanced economies in the mid 2020s, passive decay alone implies a half-life of about seven quarters for AI capacity versus roughly eleven years for conventional capital, helping rationalise boom and bust patterns in data centre investment. Because the optimal AI stock depends sensitively on its effective user cost, shifts that lower the effective depreciation of installed compute through better utilisation, repairability, cooling, and resale value can have large welfare effects without implying slower frontier innovation. Quantitatively, a two percentage point reduction in quarterly AI depreciation raises welfare by 0.36\% in consumption equivalent terms, while an equal sized compute tax reduces the steady state AI stock by around one fifth.
\end{abstract}
