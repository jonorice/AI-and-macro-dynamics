...
%% Author shortcuts
%% ------------------------------------------------------------
% Fixed: Wrapped math commands in \ensuremath{} to ensure they work correctly in both text and math mode.
...
\begin{abstract}
Specialised AI hardware becomes economically obsolete far faster than conventional capital. This paper embeds that distinction in a tractable growth model with two capital stocks, conventional capital and AI capacity interpreted as installed compute used to deploy existing models. AI capacity both raises productivity in goods production and supplies digital services that are cheap to use once installed but constrained by installed capacity. The steady state is pinned down by user cost conditions, and local dynamics feature two adjustment modes tied closely to the survival rates of the two stocks. Calibrated to advanced economies in the mid 2020s, AI capacity adjusts with a half life of about seven quarters, while conventional capital adjusts over roughly eleven years, helping rationalise boom and bust patterns in data centre investment. Because the optimal AI stock depends sensitively on its effective user cost, shifts that lower the effective depreciation of installed compute through better utilisation, repairability, cooling, and resale value can have large welfare effects without implying slower frontier innovation. Quantitatively, a two percentage point reduction in quarterly AI depreciation raises welfare by 0.36\% in consumption equivalent terms, while an equal sized compute tax reduces the steady state AI stock by around one fifth.
\end{abstract}
...
To study dynamics, we log-linearise the Euler block together with the capital-accumulation equations around the steady state. This yields a two-dimensional linear system in the log deviations of K and M whose Jacobian collects the partial elasticities of the user-cost conditions with respect to the two capital stocks. The eigenvalues of this system provide the natural general equilibrium (GE) adjustment speeds of the model. In a calibration to advanced economies in the mid-2020s, the fast eigenvalue lies close to the survival rate of AI hardware and the slow eigenvalue lies close to the survival rate of conventional capital. Section 6 confirms this intuition, showing that the two eigenvalues are numerically very close to the survival rates of K and M, meaning hardware parameters effectively determine the adjustment speeds. As a result, the GE transition paths for AI capacity, tangible capital and output are well approximated by simple geometric laws of motion based on the two depreciation rates. These geometric laws serve as transparent partial equilibrium (PE) benchmarks: they make clear that hardware durability alone can generate a six-to-one gap between the half-life of AI-investment booms and that of conventional capital deepening.
...
% Fixed: Added labels eq:K_law and eq:M_law as they are referenced in Appendix B (from initial prompt).
\begin{align}
...
\medskip
% Fixed: Added labels eq:euler_K and eq:euler_M as they are referenced in Appendix B (from initial prompt).
...
% Fixed: Replaced \star with * throughout for consistency with preamble definitions.
...
\]
so the quarterly survival rate of installed AI capacity is about $1-\delta_M\approx 0.905$. Sections~\ref{sec:dynamics} and~\ref{sec:policy} show that this rate plays a central role in governing both the fast GE adjustment mode and the half-life of hardware-driven booms.
...
% Fixed: Added label tab:baseline_calib as it is referenced in Appendix B (from initial prompt).
...
% Fixed: Replaced Unicode ≈ with \approx
...
% Fixed: Replaced \star with * throughout for consistency.
...
% Fixed: Added \tag{18} to continue the numbering sequence from previous sections.
\begin{equation}
\begin{bmatrix}
k_{t+1} \\
m_{t+1}
...
% Fixed: Changed \varphi to \phi for consistency with the model definition in Section 3.
...
$Y^{*}/K^{*}$ and $Y^{*}/M^{*}$. Appendix~B summarises the linearisation and reports the resulting Jacobian and eigenvalues; here we focus on their economic content.
...
% Fixed: Changed \varphi to \phi for consistency.
...
\]
% Fixed: Replaced \simeq with \approx for better typography.
In the baseline calibration of Section~\ref{sec:calibration}, the fast eigenvalue $\lambda_1$
is very close to the survival rate of AI capacity, $1-\delta_M \approx 0.905$, while the slow
eigenvalue $\lambda_2$ lies close to the survival rate of conventional capital,
$1-\delta \approx 0.985$. Appendix~B reports the exact numerical values and the
corresponding eigenvectors. Economically, the system decomposes into a fast mode driven
primarily by the high depreciation rate of AI capacity and a slow mode associated with the
...
\end{equation}
These PE half-lives are numerically very close to the GE half-lives obtained from
$\lambda_1$ and $\lambda_2$, confirming that depreciation-driven geometric adjustment is an
accurate guide to the true transition speeds in this calibration. Appendix~B quantifies the
gaps in levels and half-lives.
...
response of AI capacity and tangible capital to a one-off 1\% increase in $M_t$ at date $0$. Unlike the GE system, this PE benchmark holds investment fixed at steady-state replacement levels and therefore abstracts from endogenous movements in consumption and marginal utility; the comparison below therefore measures how large the GE feedbacks are in the baseline calibration. The solid lines show the GE impulse responses, based on the linear system
\eqref{eq:GE_system}; the circle markers show the PE geometric paths
$m_t = (1-\delta_M)^t m_0$ and $k_t = (1-\delta)^t k_0$. 
...
steady state following a 1\% increase in $M_0$. Solid lines: GE dynamics from the corrected linearisation (Section~\ref{sec:dynamics}). Circle markers: PE geometric benchmark
\eqref{eq:PE_recursions}. The GE path for $M$ adjusts faster than the PE benchmark because the planner re-optimises investment endogenously; for $K$, the PE benchmark is zero by construction (only $M_0$ is shocked), whereas in GE the cross-effect from $M$ raises $K$ temporarily before the slow eigenvalue governs the return to steady state.
\end{minipage}
\end{figure}

For AI capacity, the GE path lies almost exactly on the PE benchmark, decaying with a
half-life very close to $t^{M}_{1/2}$ in \eqref{eq:PE_half_lives}. For tangible capital, the GE
path is more curved in the very short run because the initial disturbance originates in $M_0$
rather than $K_0$, but the medium-run persistence is again governed by the slow root and
closely tracks the geometric benchmark. This confirms that for small shocks around the steady
state, depreciation rates alone already capture most of the relevant dynamics.
...
The new fast eigenvalue, call it $\lambda^{\prime}_1$, moves closer to one because the survival
rate of AI hardware has increased, while the slow eigenvalue changes little.
...
\approx 0.0753$). The GE and PE paths again lie very close together. The half-life of the gap
...
two-percentage-point fall in $\delta_M$. Solid line: GE dynamics with Jacobian
re-evaluated at $(K^{**},M^{**})$. Circle markers: PE benchmark
\eqref{eq:PE_longevity}, in which the gap closes at the new survival rate $1-\delta_M+\Delta$.
The GE path adjusts substantially faster than the PE benchmark because the planner front-loads AI investment in response to the permanent longevity improvement.
\end{minipage}
\end{figure}
...
In the GE system, the same qualitative pattern emerges. Starting from a low $M_0$ and a
correspondingly low $K_0$, the AI stock rises rapidly towards its steady state along the fast
mode, while tangible capital deepens more slowly along the slow mode. Because the fast
eigenvalue $\lambda_1$ is close to $1-\delta_M$, the GE diffusion of $M_t$ is again well
approximated by \eqref{eq:PE_diffusion}. Figure~\ref{fig:diffusion_paths} compares the GE
and PE paths.
...
The PE benchmark converges substantially faster than the GE path because it holds investment fixed at steady-state replacement levels, whereas the GE planner optimally smooths the build-up of both capital stocks over a longer horizon.
\end{minipage}
\end{figure}

\subsection{Interpretation}

Across these experiments, the message is consistent. The log-linear GE dynamics of the
two-asset system can be summarised by two eigenvalues: a fast one, closely tied to the
high depreciation rate of AI hardware, and a slow one, tied to the much lower depreciation
rate of conventional capital. Because the corresponding eigenvectors are only weakly tilted
by the production and services cross-effects, the half-lives implied by the GE system are
numerically extremely close to the half-lives of the geometric PE benchmarks. This validates
the use of those benchmarks in the policy analysis of Section~\ref{sec:policy}, while making
clear that they are approximations to an underlying GE structure rather than free-standing
assumptions.
...
% Fixed: Updated tag numbering to (24) to follow sequentially from the previous section.
\begin{equation}
...
% Fixed: Replaced \star with * for consistency.
...
% Fixed: Updated tag numbering to (25).
\begin{equation}
\Omega(\Delta)
...
and the binding-capacity regime. The derivation approximates the transition of $M_t$ by the
fast GE mode; in the baseline calibration the associated eigenvalue
$\lambda_1\approx 0.905$ is numerically almost identical to the survival rate
$1-\delta_M\approx 0.905$ (Section~\ref{sec:dynamics}), so expressing the speed in terms of
$\delta_M$ is an accurate simplification. The numerator captures the discounted effect of a
...
% Fixed: Replaced \simeq with \approx.
...
the first few years: the bulk of the adjustment in AI capacity is complete within roughly two
half-lives (about three to four years), after which the economy is close to its new steady
state.

\begin{figure}[t]
	\centering
	\caption{\textbf{Steady-state effects of a hardware tax (or subsidy)}}
% Fixed: Removed leading non-standard whitespace. Removed .png extension.
    \vspace{0.35cm}
	\includegraphics[width=\textwidth]{fig4}
	\label{fig4}

	{\footnotesize
% Fixed: Replaced \star with *.
...
against AI-driven credit booms by introducing a counter-cyclical capital buffer. The fast GE
eigenvalue $\lambda_1$ in Section~\ref{sec:dynamics}, which is numerically close to the survival
rate $1-\delta_M$, pins down the half-life of deviations in AI capacity:
\[
t_{1/2}
=\frac{\ln 0.5}{\ln \lambda_1}
\;\approx\;
\frac{\ln 0.5}{\ln(1-\delta_M)}
\approx 6.9\ \text{quarters},
\]
or roughly one and three-quarter years in the baseline calibration. A simple rule that
accumulates capital buffers while $\dot M_t>0$ and releases them automatically after one
half-life would therefore line up the prudential cycle with the natural economic decay of
hardware booms. Unlike traditional Basel~III buffers, which often release with multi-year
...
in wage-setting or price-setting conditions. The fast and slow eigenvalues of the core
two-asset system, and the associated benchmark half-lives, are therefore unchanged.
...
with elasticity $\nu>1$:
% Fixed: Removed leading non-standard whitespaces in the equation.
\[
Y_t
...
\begin{figure}[H]
% Fixed: Removed leading non-standard whitespaces.
    \centering
    \caption{\textbf{Wage premium after hardware-longevity shock}}
    \vspace{0.3cm}
% Fixed: Removed .png extension.
...
% Fixed: Updated tag numbering to (26) to follow sequentially from Section 6. Removed leading whitespaces.
\begin{equation}
\pi_{t}
...
that are numerically very close to the survival rates of the two capital stocks. Simple
geometric laws of motion are therefore accurate guides to adjustment speeds and make
policy experiments essentially pencil-and-paper.

Three quantitative lessons emerge. First, hardware durability is the dominant state variable.
It determines the fast eigenvalue of the GE system and hence the benchmark half-life of
AI-investment booms, dictates how much capacity firms wish to hold, and controls the size
of the digital block. In the baseline calibration, the fast mode associated with AI capacity has
a half-life of roughly seven quarters, while the slow mode associated with conventional
capital has a half-life of more than a decade. Second, the elasticity with which digital
...
prudential timing: because the fast GE eigenvalue is very close to the hardware survival
rate, AI-investment booms unwind with a half-life of about seven quarters, so a
...
\paragraph{Why the feasibility equation matters}
In the steady state, $I^{K*}=\delta K^{*}$ and $X^{*}=\delta_M M^{*}$, but these equalities do not
hold along transitions. Using the steady-state identities inside the non-steady-state feasibility
equation would mechanically suppress an important channel by which investment decisions move
$t$-dated consumption and therefore the co-state $\lambda_t$. Equations \eqref{eq:B_resource}--\eqref{eq:B_eulerM}
retain the correct dependence of $\lambda_t$ on investment, which is essential for the local dynamics.

\subsection{Reduced state transition and eigenvalues}

Let $k_t\equiv\ln K_t-\ln K^{*}$ and $m_t\equiv\ln M_t-\ln M^{*}$ denote log deviations from the
steady state, and collect them in $x_t\equiv(k_t,m_t)^{\prime}$. Log-linearising
\eqref{eq:B_resource}--\eqref{eq:B_eulerM} around the steady state yields a linear rational-expectations
system in the predetermined states $(k_t,m_t)$ and the associated jump variables (consumption and investment
controls). The unique stable solution implies a two-dimensional law of motion for the predetermined stocks,
\begin{equation}
x_{t+1} = J\,x_t,
\label{eq:B_GE_reduced}
\end{equation}
where $J$ is the reduced-form Jacobian reported in Section~\ref{sec:dynamics}. In practice, $J$ is obtained
by solving the linearised equilibrium system and selecting the stable roots, with saddle-path stability
implying exactly two eigenvalues inside the unit circle, matching the number of predetermined states.

Let $\lambda_1$ and $\lambda_2$ denote the eigenvalues of $J$, ordered so that
$0<\lambda_1\le\lambda_2<1$. The corresponding half-lives are
\begin{equation}
t_{1/2}(\lambda_i)=\frac{\ln 0.5}{\ln\lambda_i},\qquad i=1,2.
\label{eq:B_halflife}
\end{equation}

Under the baseline calibration in Table~\ref{tab:baseline_calib}, the computed eigenvalues are
\begin{equation}
\lambda_1 \approx 0.905,\qquad \lambda_2 \approx 0.985,
\label{eq:B_eigs_numeric}
\end{equation}
implying half-lives of roughly $6.9$ quarters for the fast mode and $44.8$ quarters for the slow mode. For reference, the corresponding depreciation-based survival rates are $1-\delta_M\approx 0.905$ and $1-\delta\approx 0.985$, so the GE and PE roots coincide to three decimals in this calibration.
This numerical proximity reflects the fact that, in the baseline calibration, depreciation dominates the diagonal terms of the local state transition, and the cross-effects through production and the services dividend are quantitatively small.
